...
function multiSelect(id) {
  let selected = checkBoxes.findIndex((el, index) => {
    if (el.id == id) {
      return true;
    }
  });

  let checked = checkBoxes.findIndex((el, index) => {
    if (el.checked) {
      return true;
    }
  });
  ...
  if (checked) {
    if (checked < selected) {
      for (let i = checked; i < selected + 1; i++) {
        checkBoxes[i].checked = true;
      }
    }

    if (checked > selected) {
      for (let i = selected; i < checked + 1; i++) {
        checkBoxes[i].checked = true;
      }
    }
  }
...
document.addEventListener("click", function (e) {
  if (e.shiftKey && e.target.closest("label")) {
    // let box = document.querySelector(`${e.target.closest("label")} input`);
    let label = e.target.closest("label").attributes.for.textContent;
    multiSelect(label);
  } else {
